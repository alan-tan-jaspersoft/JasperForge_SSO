...
?>
	...
	<div class="stats-wrapper">
		...
		<?php
			...
			$totalviews = db_query("SELECT totalcount FROM mongodb_node_counter WHERE nid = :nid",array(':nid'=>(int)$fields['nid']->content))->fetchField();
			$totalviews = $totalviews ? $totalviews : 0;
		?>
		
		<div class="views-field views-field-totalcount<?php echo ($totalviews > 0) ? ' stats-view-active' : ' stats-view'; ?>">
			<?php 
				$suffix = array("", "K", "M");
				if($totalviews == 0) { $hits = 0; } else {
				$hits = (round($totalviews/pow(1000, ($i = floor(log($totalviews, 1000)))), $totalviews >= 100000 ? 0 : 1) . $suffix[$i]); }
			?>
			
			<?php print $hits; //print $fields['totalcount']->content; ?>
			<div class="stats-label"><?php print format_plural($totalviews,'view','views'); ?></div>
		</div>
	</div>	
