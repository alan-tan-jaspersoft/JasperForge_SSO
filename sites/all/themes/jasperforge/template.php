...
function jasperforge_preprocess_page(&$vars) {
	...
	if (isset($vars['node']->type)) {
		...
		if($vars['node']->type == 'documentation_html_') {
			drupal_add_library('system','ui.resizable');
			drupal_add_js('sites/all/themes/jasperforge/js/html_docs.js');
		}
	}
}

function jasperforge_preprocess_comment(&$vars) {
	if(isset($vars['node'])){
		$vars['theme_hook_suggestions'][] = 'comment__'.$vars['node']->type;
	}
}

function jasperforge_preprocess_comment_wrapper(&$vars) {
...
function jasperforge_preprocess_region(&$vars) {
...
function jasperforge_wysiwyg_editor_settings_alter(&$settings, $context) {
   if ($context['profile']->editor == 'ckeditor') {
     $settings['skin'] = 'chris,' . base_path() . 'sites/all/libraries/ckeditor_skins/chris/';
   }
}

function jasperforge_preprocess_field(&$vars) {
